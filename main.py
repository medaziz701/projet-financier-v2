...
def main():
    ...
    # Optionally apply a nicer ttk theme if available
    ...
    # Configurer le style des Treeview APRÈS ThemedStyle (noir avec texte blanc)
    configurer_style_treeview()

    # Menubar
    menubar = tk.Menu(root)

    # Menu Fichier / Navigation de base
    ...
    # Simple home screen with quick actions
...
if __name__ == "__main__":
    # Ensure database is initialized before starting UI
    try:
        initialiser_bdd()
    except Exception:
        # If initialization fails, still attempt to open UI so user can see errors elsewhere
        pass
    # Try to run the legacy (ancien) full interface from main.py.bak if available
    ...
    if os.path.exists(legacy_path):
        try:
            ...
            with open(legacy_path, "r", encoding="utf-8", errors="ignore") as f:
                code = f.read()
            # Execute legacy code in isolated env and run its main()
